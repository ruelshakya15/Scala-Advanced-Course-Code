...
object Monads extends App {
  ...
  class Lazy[+A](value: => A) {
    ...
    def use : A = value
    def flatMap[B](f: (=>A) => Lazy[B]) = f(internalValue) //*** see how we pass fxn BYNAME(takes fxn that uses paramater byname)
  }
  object Lazy{
    def apply[A](element: => A): Lazy[A] =  new Lazy(element) // unit
  }
